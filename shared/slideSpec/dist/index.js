...
export function safePalette(p) {
    ...
    const isHex6 = (color) => !!color && /^#[0-9A-Fa-f]{6}$/.test(color);
    const primary = isHex6(p.primary) ? p.primary : "#1E40AF";
    const accent = isHex6(p.accent) ? p.accent : "#F59E0B";
    const neutral = Array.isArray(p.neutral) && p.neutral.filter(isHex6).length >= 5
        ? (() => {
            const cleaned = p.neutral.filter(isHex6).slice(0, 9);
            while (cleaned.length < 9)
                cleaned.push(DEFAULT_NEUTRAL_9[cleaned.length]);
            return cleaned;
        })()
        : [...DEFAULT_NEUTRAL_9];
...
export const SlideSpecZ = z
    ...
    const ordersByRegion = new Map();
    value.layout.anchors.forEach((a, i) => {
        const set = ordersByRegion.get(a.region) ?? new Set();
        if (set.has(a.order)) {
            ctx.addIssue({
                code: "custom",
...
export function normalizePalette(p) {
    const primary = isHex6(p.primary) ? p.primary : "#1E40AF";
    const accent = isHex6(p.accent) ? p.accent : "#F59E0B";
    const neutral = Array.isArray(p.neutral) &&
 ...
 */
export function calculateOptimalFontSize(text, maxWidth, baseFontSize = 16) {
 ...
 */
export function calculateCellDimensions(totalWidth, totalHeight, cols, rows, gutter, margin) {
 ...
 */
export function calculateRegionPosition(rowStart, colStart, rowSpan, colSpan, cellWidth, cellHeight, gutter, margin) {
